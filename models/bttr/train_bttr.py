...
# Add the models/bttr directory to path so imports work from project root
_BTTR_DIR = os.path.dirname(os.path.abspath(__file__))
if _BTTR_DIR not in sys.path:
    sys.path.insert(0, _BTTR_DIR)
...
def main():
    ...
    # Configs
    train_root = "../../CROHME/train"
    val_root = "../../CROHME/2014"
    batch_size = 16
    num_epochs = 100
    learning_rate = 1.0
    patience = 10
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with torch.no_grad():
            for fnames, imgs, masks, formulas in val_loader:
                ...
                gt_indices = formulas[0]
                pred_list = [int(x) for x in preds[0][0]]
                gt_list = [int(x) for x in gt_indices]
                exp_rate_recorder.update(pred_list, gt_list)
